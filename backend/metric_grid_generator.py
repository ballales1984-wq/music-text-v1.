...
def generate_text_from_grid(metric_grid: Dict, use_ai: bool = True, instrumental_features: Dict = None, transcription: Dict = None) -> Dict:
    ...
    for line_idx, line_info in enumerate(lines_info):
        ...
        # IMPORTANTE: NON pulire la riga generata! L'IA ha già generato testo corretto.
        # Solo pulizia minima per contrazioni rotte (It' -> It's), ma NON rimuovere contenuto
        line_text = line_text.strip()
        
        # Correggi solo contrazioni rotte comuni (formattazione minima)
        line_text = line_text.replace("It' ", "It's ").replace("It'", "It's")
        line_text = line_text.replace("I' ", "I'm ").replace("I'", "I'm")
        line_text = line_text.replace("We' ", "We're ").replace("We'", "We're")
        line_text = line_text.replace("You' ", "You're ").replace("You'", "You're")
        line_text = line_text.replace("They' ", "They're ").replace("They'", "They're")
    ...
    # IMPORTANTE: NON rimuovere righe! L'IA deve SOSTITUIRE con inglese corretto, non togliere.
    # Usa solo pulizia minima per correggere errori di formattazione (spazi, contrazioni rotte)
    # MA NON rimuovere righe duplicate - ogni riga è parte della canzone
    from text_cleaner import clean_generated_text
    
    # Pulisci solo errori di formattazione (contrazioni rotte, spazi multipli)
    # MA preserva TUTTE le righe generate
    result_lines_cleaned = []
    for line in result_lines:
        if line and len(line.strip()) >= 3:
            # Pulisci solo formattazione (contrazioni, spazi), NON rimuovere la riga
            line_cleaned = clean_generated_text(line.strip())
            # Se la pulizia ha rimosso troppo, usa originale
            if len(line_cleaned) < len(line.strip()) * 0.5:
                line_cleaned = line.strip()
            result_lines_cleaned.append(line_cleaned)
        elif line and len(line.strip()) > 0:
            # Mantieni anche righe corte
            result_lines_cleaned.append(line.strip())
    
    result_lines = result_lines_cleaned
    
    # Assicurati che abbiamo TUTTE le righe generate (nessuna rimossa)
    if len(result_lines) < len(previous_lines):
        logger.warning(f"⚠️  Righe perse: {len(previous_lines)} -> {len(result_lines)}, ripristino TUTTE")
        result_lines = [line.strip() for line in previous_lines if line and len(line.strip()) >= 3]
...
def _generate_line_text(line_slots: List[Dict], song_context: Dict, song_theme: str, use_ai: bool = True, previous_lines: List[str] = None, line_index: int = 0, total_lines: int = 0, singing_features: Dict = None) -> str:
    ...
    if transcribed_line and len(transcribed_line.strip()) > 2:
        ...
        # IMPORTANTE: Passa previous_lines, line_index, total_lines per generare righe DIVERSE
        return _improve_existing_line(transcribed_line, song_context, song_theme, total_syllables, use_ai, previous_lines=previous_lines, line_index=line_index, total_lines=total_lines, singing_features=singing_features)
...
def _improve_existing_line(line_text: str, song_context: Dict, song_theme: str, target_syllables: int, use_ai: bool, previous_lines: List[str] = None, line_index: int = 0, total_lines: int = 0, singing_features: Dict = None) -> str:
    """
    ...
    IMPORTANTE: Anche se il testo trascritto è lo stesso, genera righe DIVERSE basandosi su:
    - Posizione della riga (line_index, total_lines)
    - Righe precedenti (previous_lines)
    - Contesto musicale specifico
    """
    from text_cleaner import clean_generated_text
    
    if not line_text or len(line_text.strip()) < 3:
        return _generate_new_line(song_context, song_theme, target_syllables, 2.0, use_ai, previous_lines, line_index, total_lines, singing_features=singing_features)
    ...
    if use_ai:
        try:
            ...
            # Costruisci contesto righe precedenti per generare righe DIVERSE
            previous_context = ""
            if previous_lines and len(previous_lines) > 0:
                previous_context = f"\n\nPREVIOUS LINES (for narrative coherence and VARIETY):\n" + "\n".join([f"Line {i+1}: {line}" for i, line in enumerate(previous_lines)])
            
            # Posizione della riga nella canzone
            line_position = ""
            if total_lines > 0:
                if line_index == 0:
                    line_position = "\n- POSITION: This is the FIRST line of the song (opening/intro) - make it engaging and unique"
                elif line_index == total_lines - 1:
                    line_position = f"\n- POSITION: This is the LAST line of the song (ending/conclusion) - make it memorable and unique"
                elif line_index < total_lines // 2:
                    line_position = f"\n- POSITION: This is line {line_index + 1} of {total_lines} (early in the song) - make it unique and different from previous lines"
                else:
                    line_position = f"\n- POSITION: This is line {line_index + 1} of {total_lines} (later in the song) - make it unique and different from previous lines"
...
5. Write a UNIQUE line that is DIFFERENT from previous lines (even if the transcribed text is the same)
6. DO NOT repeat words/phrases from previous lines - create NEW, VARIED lyrics
7. Consider this is SINGING - words should flow with the melody and rhythm
8. If the transcribed line is the same as previous lines, create a DIFFERENT variation that fits the same melody

IMPORTANT: 
- Even if the transcribed line "{line_text}" appears multiple times in the song, you MUST create a DIFFERENT English line for this position
...
- Create VARIETY: each line should be unique, even if they share the same transcribed sounds

Write ONLY the improved line (unique, no repetitions, no explanations):"""
