    ...
    try:
        ...
        if len(strong_beats) > 4:
            ...
            if len(strong_intervals) > 0:
                ...
                # Converti a float per round()
                if isinstance(avg_strong_interval, np.ndarray):
                    avg_strong_interval = float(avg_strong_interval.item())
                else:
                    avg_strong_interval = float(avg_strong_interval)
