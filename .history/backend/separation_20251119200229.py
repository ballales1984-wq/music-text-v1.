    ...
    try:
        ...
        if wav.shape[0] == 2:
            ...
            try:
                ...
                try:
                    ...
                    if TORCH_AVAILABLE:
                        ...
                        vocals = vocals_combined.reshape(1, -1)
                    ...
                    if TORCH_AVAILABLE:
                        ...
                        instrumental = instrumental_raw.reshape(1, -1)
                    ...
                    vocals_diff = (wav[0] - wav[1]) / 2
                    if TORCH_AVAILABLE:
                        vocals = vocals_diff.unsqueeze(0) if hasattr(vocals_diff, 'unsqueeze') else torch.from_numpy(vocals_diff).unsqueeze(0)
                    else:
                        vocals = vocals_diff.reshape(1, -1) if hasattr(vocals_diff, 'reshape') else np.array(vocals_diff).reshape(1, -1)
                    # BASE = Canale centrale (L+R)/2 - voce (sottrazione)
                    mix_center = (wav[0] + wav[1]) / 2
                    instrumental_raw = mix_center - vocals_diff
                    if TORCH_AVAILABLE:
                        instrumental = instrumental_raw.unsqueeze(0) if hasattr(instrumental_raw, 'unsqueeze') else torch.from_numpy(instrumental_raw).unsqueeze(0)
                    else:
                        instrumental = instrumental_raw.reshape(1, -1) if hasattr(instrumental_raw, 'reshape') else np.array(instrumental_raw).reshape(1, -1)
            ...
            if TORCH_AVAILABLE:
                ...
                instrumental = np.array(wav) * 0.1 if isinstance(wav, np.ndarray) else wav * 0.1
            
            logger.info("⚠️  File mono: voce=completo, base=attenuata")
        else:
            # Multi-canale: usa primi due canali
            vocals_raw = (wav[0] + wav[1]) / 2 if wav.shape[0] >= 2 else wav[0]
            if TORCH_AVAILABLE:
                vocals = vocals_raw.unsqueeze(0) if hasattr(vocals_raw, 'unsqueeze') else torch.from_numpy(vocals_raw).unsqueeze(0)
            else:
                vocals = vocals_raw.reshape(1, -1) if hasattr(vocals_raw, 'reshape') else np.array(vocals_raw).reshape(1, -1)
            instrumental_raw = (wav[0] - wav[1]) / 2 if wav.shape[0] >= 2 else wav[0] * 0.1
            if TORCH_AVAILABLE:
                instrumental = instrumental_raw.unsqueeze(0) if hasattr(instrumental_raw, 'unsqueeze') else torch.from_numpy(instrumental_raw).unsqueeze(0)
            else:
                instrumental = instrumental_raw.reshape(1, -1) if hasattr(instrumental_raw, 'reshape') else np.array(instrumental_raw).reshape(1, -1)
