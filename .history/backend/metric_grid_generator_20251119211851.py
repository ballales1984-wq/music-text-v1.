...
def _is_valid_english_word(word: str) -> bool:
    """Verifica se una parola è inglese valida."""
    ...
    # Fallback: euristica base
    # Parole comuni non-inglesi da escludere
    ...
    # Se contiene solo vocali ripetute, probabilmente non è inglese
    if re.match(r'^[aeiou]+$', word) and len(word) <= 3:
        return False
...
def generate_text_from_grid(metric_grid: Dict, use_ai: bool = True, instrumental_features: Dict = None, transcription: Dict = None) -> Dict:
    ...
    for line_idx, line_info in enumerate(lines_info):
        ...
        line_text = clean_generated_text(line_text.strip())
    ...
    logger.info(f"✅ Testo generato: {len(result_lines)} righe, {len(replacements)} sostituzioni, tema: {song_theme}")
...
def _generate_line_text(line_slots: List[Dict], song_context: Dict, song_theme: str, use_ai: bool = True, previous_lines: List[str] = None, line_index: int = 0, total_lines: int = 0) -> str:
    """
    ...
    if transcribed_line and len(transcribed_line.strip()) > 2:
        ...
        return _improve_existing_line(transcribed_line, song_context, song_theme, total_syllables, use_ai)
    
    # Se non c'è testo trascritto, genera nuova riga basata su tema e contesto
    return _generate_new_line(song_context, song_theme, total_syllables, line_duration, use_ai, previous_lines, line_index, total_lines)


def _improve_existing_line(line_text: str, song_context: Dict, song_theme: str, target_syllables: int, use_ai: bool) -> str:
    """
    ...
    if not line_text or len(line_text.strip()) < 3:
        return _generate_new_line(song_context, song_theme, target_syllables, 2.0, use_ai, None, 0, 0)
    ...
    if use_ai:
        try:
            ...
            prompt = f"""You are listening to a song and writing the English lyrics that match what you hear.

TRANSCRIBED LINE (what you hear being sung): "{line_text}"

CRITICAL REQUIREMENTS:
1. Have EXACTLY {target_syllables} syllables (currently has {existing_syllables}) - COUNT CAREFULLY
2. Match the song theme: {song_theme}
3. Fit the tempo: {tempo} BPM, mood: {mood}
...
IMPORTANT: 
- If the transcribed line is repetitive, create a SINGLE unique line instead
- Transform the sounds/words you hear into proper English lyrics
...
def _generate_new_line(song_context: Dict, song_theme: str, target_syllables: int, duration: float, use_ai: bool, previous_lines: List[str] = None, line_index: int = 0, total_lines: int = 0) -> str:
    """
    ...
    if use_ai:
        try:
            return _generate_line_with_ai(song_context, song_theme, target_syllables, duration, previous_lines, line_index, total_lines)
...
def _generate_line_with_ai(song_context: Dict, song_theme: str, target_syllables: int, duration: float, previous_lines: List[str] = None, line_index: int = 0, total_lines: int = 0) -> str:
...
- Mood/emotion: {mood}
- Energy level: {energy}
- This line duration: {duration:.2f} seconds
{line_position}{previous_context}

YOUR TASK:
Write ONE line of English song lyrics that:
...
5. Sounds like actual song lyrics someone would sing to this music
...
IMPORTANT: Write ONE unique line - NO repetitions, NO duplicate words/phrases within the line.
