    ...
    for attempt in range(3):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ Ollama disponibile - Modello selezionato: {OLLAMA_MODEL}")
                break
...
def generate_english_text_from_vocals(transcription_data: Dict) -> str:
    ...
    # Prova sempre con AI prima (Ollama o OpenAI)
    result = None
    
    if OLLAMA_AVAILABLE:
        try:
            logger.info(f"🤖 Tentativo generazione con Ollama ({OLLAMA_MODEL})...")
            if has_clear_words and raw_text:
                result = _enhance_with_ollama(raw_text + context_info)
            else:
                result = _generate_with_ollama(input_text + context_info)
            
            if result and len(result) > 30:
                logger.info(f"✅ Testo generato con Ollama: {len(result)} caratteri")
                return result
            else:
                logger.warning(f"⚠️ Ollama ha generato testo troppo corto: {len(result) if result else 0} caratteri")
        except Exception as e:
    ...
    # Fallback solo se AI non disponibile o completamente fallita
    if not result or len(result) < 20:
        logger.warning("⚠️ AI non disponibile o fallita - uso fallback migliorato basato sul testo")
...
def _enhance_with_ollama(text: str) -> str:
    """Migliora testo con Ollama - prompt migliorato per testi unici."""
    import requests
    
    # Estrai parole chiave dal testo per personalizzare
    words = text.lower().split()[:20]  # Prime 20 parole
    key_words = [w for w in words if len(w) > 3][:5]  # Parole significative
    
    prompt = f"""You are a professional song lyricist. Transform this transcribed vocal text into beautiful, unique, poetic English song lyrics.

CRITICAL: Generate UNIQUE lyrics based on THIS SPECIFIC transcription. Do NOT use generic templates. Each song must be different.

Original transcribed text from the vocal track:
{text}

Key words detected: {', '.join(key_words) if key_words else 'various'}

Requirements:
- Generate UNIQUE lyrics that reflect THIS specific song
- Be in English
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                "stream": False,
                "options": {
                    "temperature": 0.8,  # Più creatività per testi unici
                    "top_p": 0.9,
                    "num_predict": 800,  # Più testo generato
                    "repeat_penalty": 1.2  # Evita ripetizioni
                }
            },
...
def _generate_with_ollama(sounds: str) -> str:
    """Genera testo da suoni vocali con Ollama - prompt migliorato per testi unici."""
    import requests
    
    # Analizza i suoni per estrarre pattern
    sound_words = sounds.lower().split() if sounds else []
    unique_sounds = list(set([w for w in sound_words if len(w) > 2]))[:10]
    
    prompt = f"""You are a creative song lyricist. Generate beautiful, UNIQUE, poetic English song lyrics based on vocal sounds and melodies.

CRITICAL: Generate UNIQUE lyrics that are SPECIFIC to these vocal sounds. Do NOT use generic templates. Each song must be completely different based on the sounds.

Vocal sounds and melodies detected:
{sounds}
...
- Tell a story or express emotions that match the mood of the sounds
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                "stream": False,
                "options": {
                    "temperature": 0.85,  # Più creatività
                    "top_p": 0.9,
                    "num_predict": 800,  # Più testo
                    "repeat_penalty": 1.2  # Evita ripetizioni
                }
            },
            timeout=120  # Timeout aumentato
